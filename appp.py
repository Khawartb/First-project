...
def assistant_chat_css(logo_file):
    ...
    """
        ...
        st.error(f"Error creating CSS: {e}")
        return ""
...
# Paths to your images - use relative paths and place images in same directory as script
logo_path = "logo.jpeg"
background_path = "bgimage.png"

# Knowledge script (strict)
prompt = """
You are a Courses and informational Guide at Yashfeen Skills Development Center.\
You are ai assisted bot of Yashfeen Skills Development Center (YSDC) institute.\
...
INSTITUTE OVERVIEW:
 ...
 - Name: Yashfeen Skills Development Center (YSDC)
 ...
 - Social Media: Follow us on Facebook, Instagram, and Whatsapp for more updates and latest news.
...
_"Sorry But I can't answer that. I'm only here to help you with any query related to Yashfeen Skills Development Center or its courses."_  

"""

# Verify images exist
if not os.path.exists(logo_path):
    st.error(f"Logo image not found at: {os.path.abspath(logo_path)}")
if not os.path.exists(background_path):
    st.error(f"Background image not found at: {os.path.abspath(background_path)}")
...
with st.sidebar:
    ...
    if os.path.exists(logo_path):
        st.image(logo_path, use_container_width=150)
    else:
...
Assistant:""".strip()
    ...
    chain = ConversationChain(
        llm=llm,
        memory=memory,
        prompt=prompt_template.partial(script=prompt),
        verbose=False
    )
else:
